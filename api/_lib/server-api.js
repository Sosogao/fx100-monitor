...
var rawParameterDefinitions = [
  ...
  { category: "Position Limits", label: "Soft OI Factor Long", key: "maxOpenInterestFactorLong", unit: "%" },
  { category: "Position Limits", label: "Soft OI Factor Short", key: "maxOpenInterestFactorShort", unit: "%" },
  { category: "Position Limits", label: "Max OI Cap Long", key: "maxOpenInterestCapLongUsd", unit: "$" },
  { category: "Position Limits", label: "Max OI Cap Short", key: "maxOpenInterestCapShortUsd", unit: "$" },
...
async function readOracleMidPrice(provider, token) {
  ...
  return raw !== void 0 ? usdValue(raw) : void 0;
}
async function readOracleMidPriceRaw(provider, token) {
  try {
...
async function loadLiveState() {
  ...
  try {
    ...
    if (positionCount > 0) {
      ...
      const longCollateralByMarket = /* @__PURE__ */ new Map();
      const shortCollateralByMarket = /* @__PURE__ */ new Map();
...
function buildMarkets(liveState) {
  ...
  const discovered = liveState.onchainMarkets.length > 0 ? liveState.onchainMarkets : basefx100Sepolia0312.markets.map((market) => ({
    ...
    poolUsdWithoutPnl: round((market.askDepthUsd + market.bidDepthUsd) / 1e3, 2),
    positionCollateralUsd: 0,
  ...
  const markets = discovered.map((marketState) => {
    ...
    const poolCollateralAmount = marketState.poolUsdWithoutPnl > 0 ? marketState.poolUsdWithoutPnl : marketState.poolCollateralAmount > 0 ? marketState.poolCollateralAmount : marketState.collateralVaultBalance;
    ...
    const availableLongUsd = marketState.availableLongUsd > 0 ? marketState.availableLongUsd : round(Math.max(0, Math.min(
    ...
    const availableShortUsd = marketState.availableShortUsd > 0 ? marketState.availableShortUsd : round(Math.max(0, Math.min(
    ...
    return {
      ...
      maxOpenInterestLongUsd: marketState.maxOpenInterestLongUsd,
      maxOpenInterestShortUsd: marketState.maxOpenInterestShortUsd,
      maxOpenInterestFactorLongPct: marketState.maxOpenInterestFactorLongPct,
      maxOpenInterestFactorShortPct: marketState.maxOpenInterestFactorShortPct,
...
function buildParameters(markets, liveState) {
  ...
  return markets.map((market) => {
    ...
    const current = {
      ...
      maxOpenInterestCapLongUsd: market.maxOpenInterestLongUsd,
      maxOpenInterestCapShortUsd: market.maxOpenInterestShortUsd,
    ...
    assignSource(currentSources, [
      ...
      "maxOpenInterestFactorLong",
      "maxOpenInterestFactorShort",
      "maxOpenInterestCapLongUsd",
      "maxOpenInterestCapShortUsd",
      "reserveFactor",
      "reserveFactorLong",
      "reserveFactorShort",
      "oiReserveFactorLong",
